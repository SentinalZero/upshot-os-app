...
serve(async (req) => {
  ...
  try {
    ...
    const review = objectValue(metadata.human_review);
    ...
    if (!scopes.includes("https://www.googleapis.com/auth/gmail.compose")) return respond({ error: "Google connection does not include Gmail send permission. Reconnect Google Workspace." }, 409);

    let token = await accessToken(admin, integration, secrets);
    let gmailResponse = await fetch("https://gmail.googleapis.com/gmail/v1/users/me/drafts/send", {
      method: "POST",
    ...
    if (gmailResponse.status === 401) {
      ...
      gmailResponse = await fetch("https://gmail.googleapis.com/gmail/v1/users/me/drafts/send", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        body: JSON.stringify({ id: draftId }),
      });
    ...
    let sentPayload: Record<string, any> = {};
    try { sentPayload = responseText ? JSON.parse(responseText) : {}; } catch { sentPayload = {}; }
    if (!gmailResponse.ok) return respond({ error: `Gmail send failed (${gmailResponse.status})`, details: responseText.slice(0, 500) }, 502);
    ...
    if (updateError) return respond({ error: `Email sent, but Upshot could not save the send record: ${updateError.message}` }, 500);
