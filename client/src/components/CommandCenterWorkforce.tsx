    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-2xl border border-gold/25 bg-gradient-to-br from-gold/12 via-surface to-background/70 shadow-[0_28px_90px_-60px_oklch(0.72_0.15_75)]">
        <div className="flex flex-wrap items-start justify-between gap-5 border-b border-gold/15 p-5 sm:p-6">
          <div className="max-w-2xl"><div className="flex items-center gap-2 text-gold"><Sparkles className="h-4 w-4" /><p className="text-[10px] font-mono uppercase tracking-[0.2em]">Command Center</p></div><h2 className="mt-2 font-display text-2xl font-semibold sm:text-3xl">Your workforce, visible and under control</h2><p className="mt-3 text-xs leading-5 text-muted-foreground">See what every Specialist is doing, what needs your judgment, and what has already been completed. Nothing executes outside the authority you configured.</p></div>
        ...
        <div className="grid grid-cols-2 gap-px bg-gold/10 sm:grid-cols-4">
          ...
          <CommandMetric label="Assigned to you" value={reviewCount} detail={reviewCount ? "Judgment requested" : "Nothing waiting"} alert={reviewCount > 0} />
          <CommandMetric label="Completed today" value={metrics.successfulExecutionsToday} detail={metrics.executionsToday > 0 ? `${metrics.successRateToday}% success rate` : "Not enough activity yet"} />
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 items-start gap-8 xl:grid-cols-[1.35fr_0.85fr]">
        <section className="overflow-hidden rounded-2xl border border-subtle bg-surface">
          <div className="flex items-center justify-between border-b border-subtle p-5"><div><p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Workforce</p><strong className="font-display text-sm">Digital Specialists</strong></div><span className="flex items-center gap-1.5 rounded-full bg-[oklch(0.75_0.18_155/15%)] px-2.5 py-1 text-[10px] font-mono font-semibold text-[oklch(0.75_0.18_155)]"><span className="h-1.5 w-1.5 rounded-full bg-[oklch(0.75_0.18_155)]" />{metrics.activeSpecialists} Online</span></div>
        ...
        <section className="overflow-hidden rounded-2xl border border-subtle bg-surface xl:sticky xl:top-24">
          <div className="flex items-center justify-between border-b border-subtle p-5"><div><p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Live Operations</p><strong className="font-display text-sm">Workforce Timeline</strong></div><Radar className="h-4 w-4 text-gold" /></div>
      ...
      <section className="flex flex-col gap-4 rounded-2xl border border-subtle bg-surface p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gold/10 text-gold"><Link2 className="h-4 w-4" /></div><div><p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Business Systems</p><p className="mt-1 text-xs text-muted-foreground">{connectionCounts.connected} connected · {connectionCounts.selected} selected · {connectionCounts.attentionRequired} need attention</p></div></div>
        <Link href="/app/connections" className="text-xs font-semibold text-gold hover:underline">Manage systems →</Link>
      </section>
    </div>
  );
}
...
function SpecialistCard({ specialist, summary, capabilityCount, onOpen }: { specialist: DigitalSpecialist; summary?: SpecialistOperationalSummary; capabilityCount: number; onOpen: (specialist: DigitalSpecialist) => void }) {
  ...
  return <button type="button" onClick={() => onOpen(specialist)} className={`group w-full rounded-xl border p-5 text-left transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold ${deactivated ? "border-dashed border-subtle bg-background/20 opacity-75 hover:opacity-100" : reviewCount > 0 ? "border-[oklch(0.75_0.18_75/35%)] bg-[oklch(0.75_0.18_75/5%)] hover:bg-[oklch(0.75_0.18_75/8%)]" : "border-subtle bg-background/35 hover:-translate-y-0.5 hover:border-gold/25 hover:bg-background/55 hover:shadow-lg"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-display text-lg font-semibold">{specialist.name}</p><p className="truncate text-xs text-muted-foreground">{specialist.role_name || "Digital Specialist"}</p></div><span className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-mono font-semibold ${status.badgeClass}`}><span className={`h-1.5 w-1.5 rounded-full ${status.dotClass} ${state === "working" && !deactivated ? "animate-pulse" : ""}`} />{status.label}</span></div><div className="mt-5 rounded-lg border border-subtle bg-surface/65 p-3"><div className="flex items-center justify-between gap-2"><p className="text-[9px] font-mono uppercase tracking-wider text-muted-foreground">Current Work</p>{reviewCount > 0 && <span className="text-[9px] font-mono font-semibold uppercase tracking-wider text-[oklch(0.78_0.16_75)]">Your review requested</span>}</div><p className="mt-1 line-clamp-2 text-xs font-medium">{deactivated ? "Deactivated. No new work will be assigned." : summary?.currentJob || "Ready and monitoring for work"}</p></div><div className="mt-4 grid grid-cols-3 gap-3"><CardMetric label="Jobs Today" value={summary?.completedToday || 0} /><CardMetric label="Capabilities" value={capabilityCount} /><CardMetric label="Review" value={reviewCount} alert={reviewCount > 0} /></div><div className="mt-4 flex items-center justify-between border-t border-subtle pt-4 text-[10px] text-muted-foreground"><span>{specialist.industry_name || "General Operations"}</span><span className="flex items-center gap-1.5 font-semibold text-foreground/70">{deactivated ? "Manage specialist" : "Open command profile"}<ChevronRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5 group-hover:text-gold" /></span></div></button>;
}
...
function TimelineItem({ item, specialistName, onOpen }: { item: ActivityLog; specialistName?: string; onOpen: (item: ActivityLog) => void }) {
  ...
  const content = <><div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${isWarning ? "bg-[oklch(0.62_0.22_25/12%)] text-[oklch(0.75_0.18_25)]" : isSuccess ? "bg-[oklch(0.75_0.18_155/12%)] text-[oklch(0.75_0.18_155)]" : "bg-gold/10 text-gold"}`}><Icon className="h-3.5 w-3.5" /></div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className="text-xs font-semibold leading-5">{item.title || item.message || "Operational activity"}</p><span className="whitespace-nowrap text-[9px] font-mono text-muted-foreground">{formatActivityTime(item.created_at)}</span></div><p className="mt-0.5 line-clamp-2 text-[11px] text-muted-foreground">{item.description || item.message || item.activity_type || item.event_type || "Activity recorded"}</p><div className="mt-1.5 flex items-center justify-between gap-3">{specialistName ? <p className="text-[10px] text-gold">{specialistName}</p> : <span />}{executionId && <span className="text-[9px] font-mono uppercase tracking-wider text-muted-foreground">View details</span>}</div></div>{executionId && <ChevronRight className="mt-2 h-4 w-4 shrink-0 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:text-gold" />}</>;
  return executionId ? <button type="button" onClick={() => onOpen(item)} className="group flex w-full gap-3 px-5 py-4 text-left transition-colors hover:bg-background/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-gold">{content}</button> : <div className="flex gap-3 px-5 py-4">{content}</div>;
}
