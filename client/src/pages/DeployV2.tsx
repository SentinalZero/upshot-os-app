  ...
  const handleDeploy = async () => {
    ...
    const config: DeployConfig = {
      ...
      tasks: tasks.map(task => ({
        ...
        description: `Assigned work: ${task}`,
      })),
    };
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-8 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              ...
              <h1 className="max-w-3xl font-display text-4xl font-semibold tracking-tight lg:text-5xl">
            ...
            <div className="text-left lg:text-right">
              ...
              <div className="mt-2 flex gap-1.5">
                {steps.map((_, index) => (
                  <span key={index} className={`h-1.5 w-9 rounded-full transition-colors ${index <= step ? "bg-gold" : "bg-subtle"}`} />
                ))}
              </div>
            </div>
          </div>
            ...
            <section className="min-h-[520px] overflow-hidden rounded-3xl border border-subtle bg-surface/60 shadow-2xl shadow-black/10">
                        ...
                        <div>
                          ...
                          <div className="grid gap-3 sm:grid-cols-2">
                            {prototypeData.roles.map(item => (
                              <Choice key={item.name} selected={role.name === item.name} onClick={() => chooseRole(item)}>
                                <span>{item.name}</span>
                              </Choice>
                            ))}
                          </div>
                    ...
                    {step === 2 && (
                      <div className="grid gap-3 sm:grid-cols-2">
                        {industryData.systems.map(app => (
                          <Choice key={app} selected={apps.includes(app)} onClick={() => toggle(app, apps, setApps)}>
                            <span>{app}</span>
                            <span className="ml-auto text-[10px] font-mono uppercase text-muted-foreground">{apps.includes(app) ? "Connected" : "Connect"}</span>
                          </Choice>
                        ))}
                      </div>
                    )}
                    ...
                    {step === 4 && (
                      <div className="space-y-6">
                        <div className="flex items-center gap-4 rounded-2xl border border-gold/20 bg-gold/5 p-5">
                          <div className="grid h-14 w-14 place-items-center rounded-2xl bg-gold text-xl font-semibold text-background">
                          ...
                          <div className="flex-1">
                            ...
                            <input
                              ...
                              className="mt-1 w-full border-0 bg-transparent p-0 font-display text-2xl font-semibold outline-none"
                              aria-label="Digital Specialist name"
                            />
              ...
              <div className="flex items-center justify-between border-t border-subtle px-6 py-5 lg:px-8">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm text-muted-foreground transition-colors hover:text-foreground disabled:opacity-30"
                >
                ...
                {step < steps.length - 1 ? (
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-gold px-5 py-3 text-sm font-semibold text-background transition-transform hover:-translate-y-0.5 disabled:translate-y-0 disabled:opacity-40"
                  >
                    Continue <ArrowRight className="h-4 w-4" />
                  </button>
                ) : deployed ? (
                  <button onClick={() => navigate("/app")} className="inline-flex items-center gap-2 rounded-xl bg-emerald-400 px-5 py-3 text-sm font-semibold text-background">
            ...
            <aside className="space-y-4 lg:sticky lg:top-24">
              <div className="overflow-hidden rounded-3xl border border-subtle bg-surface">
                <div className="border-b border-subtle p-5">
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    ...
                    <div>
                      <strong className="font-display text-lg">{effectiveName}</strong>
                ...
                <div className="p-5">
                  <div className="grid grid-cols-2 gap-3">
                    {roiCards.map(card => (
                      <div key={card.label} className="rounded-2xl border border-subtle bg-background/60 p-4">
                        ...
                        <strong className={`mt-2 block text-lg font-mono ${card.accent ? "text-gold" : ""}`}>
                          {impactLoading && deployed ? "Syncing..." : card.value}
                        </strong>
                      </div>
                    ))}
                  </div>
                  <div className="mt-4 rounded-2xl border border-subtle bg-background/40 p-4">
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <strong className="text-xs">{deployed ? "Work is now being measured" : "A simple planning estimate"}</strong>
                    </div>
                    <p className="text-[11px] leading-5 text-muted-foreground">
                      {deployed
                        ? `${actualImpact.successfulExecutions.toLocaleString()} completed work items are contributing to measured impact.`
                        : "This preview is based on the work, connected tools, and approval rules you selected."}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-3xl border border-gold/15 bg-gold/5 p-5">
                ...
                <p className="mt-2 text-xs leading-5 text-muted-foreground">
                  Upshot still uses your existing specialist, connector, workflow, and approval architecture. This new experience simply starts with the user&apos;s problem instead of the engine.
                </p>
              </div>
            </aside>
          </div>
...
function Choice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex min-h-14 w-full items-center gap-3 rounded-2xl border p-4 text-left text-sm transition-all ${
        selected
          ? "border-gold/50 bg-gold/10 text-foreground shadow-lg shadow-gold/5"
          : "border-subtle bg-background/60 text-muted-foreground hover:-translate-y-0.5 hover:border-gold/30 hover:text-foreground"
      }`}
    >
      <span className={`grid h-5 w-5 shrink-0 place-items-center rounded-full border ${selected ? "border-gold bg-gold text-background" : "border-subtle"}`}>
